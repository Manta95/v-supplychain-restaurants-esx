...
RegisterServerEvent('v-businesses:GiveItem')
AddEventHandler('v-businesses:GiveItem', function(info)
    local src = source
    ...
    for _, reqItem in pairs(iteminfo.requiredItems) do
        local count = exports.ox_inventory:GetItemCount(src, reqItem.item)
        if not count or count < reqItem.amount * quantity then
            TriggerClientEvent('ox_lib:notify', src, {
    ...
    -- Retirer les items requis
    for _, reqItem in pairs(iteminfo.requiredItems) do
        exports.ox_inventory:RemoveItem(src, reqItem.item, reqItem.amount * quantity)
    end

    -- Donner l'item final
    exports.ox_inventory:AddItem(src, iteminfo.item, quantity)
...
---- BOSS MENU
---
RegisterServerEvent('esx:openSocietyStash')
AddEventHandler('esx:openSocietyStash', function(job)
    ...
    if xPlayer.job.name == job and xPlayer.job.grade_name == 'boss' then
        ...
        TriggerClientEvent('ox_lib:notify', source, {
            title = 'Accès refusé',
            description = 'Vous devez être patron pour accéder à ce coffre.',
            type = 'error'
        })
    end
end)
