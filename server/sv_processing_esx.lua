...
AddEventHandler('restaurant:orderIngredients', function(ingredient, quantity, restaurantId)
    ...
    local restaurantMoney = Config.Restaurants[restaurantId] and Config.Restaurants[restaurantId].playerMoney
    if item then
        ...
        if xPlayer then
            ...
            if restaurantMoney then
                if Money >= totalCost then
                    xPlayer.removeMoney(totalCost)
                    MySQL.Async.execute(
                        'INSERT INTO orders (owner_id, ingredient, quantity, status, restaurant_id, total_cost) VALUES (@owner_id, @ingredient, @quantity, @status, @restaurant_id, @total_cost)',
                        {
                            ['@owner_id'] = playerId,
                            ...
                            if rowsChanged > 0 then
                                TriggerClientEvent('ox_lib:notify', playerId, {
                                    ...
                                    description = string.format(
                                        'You have successfully ordered %d of %s. Total cost: $%d',
                                ...
                                TriggerClientEvent('restaurant:showOrderDetails', playerId, item.name, quantity,
                                    totalCost)
                            else
                                TriggerClientEvent('ox_lib:notify', playerId, {
                                    title = 'Order Error',
                    ...
                    TriggerClientEvent('ox_lib:notify', playerId, {
                        ...
                        description = 'Vous n\'avez pas assez de cash pour passer cette commande.',
                        type = 'error',
                        showDuration = true,
                        duration = 10000
                    })
                ...
                TriggerEvent('esx_addonaccount:getSharedAccount', 'society_' .. restaurantJob, function(account)
                    societyAccount = account
                end)
                if totalCost <= societyAccount.money then
                    societyAccount.removeMoney(totalCost)
                    TriggerClientEvent('ox_lib:notify', playerId, {
                        title = 'Commande payée par la société',
                        description = 'Votre société a été débitée de ' .. totalCost .. '$',
                        type = 'success',
                        showDuration = true,
                        duration = 10000
                    })
                    ...
                    TriggerClientEvent('ox_lib:notify', playerId, {
                        title = 'Insufficient Funds',
                        description = 'Votre société n\'a pas assez d\'argent pour passer cette commande.',
                        type = 'error',
                        showDuration = true,
                        duration = 10000
                    })
...
AddEventHandler('update:stock', function(restaurantId)
    ...
    MySQL.Async.fetchAll(
        'SELECT * FROM orders WHERE restaurant_id = @restaurant_id AND status IN ("pending", "accepted")', {
            ['@restaurant_id'] = restaurantId
        }, function(orders)
            local queries = {}
            local totalCost = 0
...
AddEventHandler('pay:driver', function(driverId, amount)
    local src = source
    local xPlayer = ESX.GetPlayerFromId(driverId)

    if xPlayer then
        xPlayer.addAccountMoney('bank', amount, "Payment for delivery")
    ...
    if player then
        ...
        if itemData then
            ...
            if amountNum and amountNum > 0 then
                player.addInventoryItem(itemData.name, amountNum)

                MySQL.Async.execute(
                    'UPDATE stock SET quantity = quantity - @amount WHERE restaurant_id = @restaurant_id AND ingredient = @ingredient',
                    {
                        ['@restaurant_id'] = restaurantId,
...
AddEventHandler('warehouse:acceptOrder', function(orderId, restaurantId)
    ...
    MySQL.Async.fetchAll('SELECT * FROM orders WHERE id = @id', {
        ...
        MySQL.Async.fetchAll('SELECT quantity FROM warehouse_stock WHERE ingredient = @ingredient', {
            ...
            if not stockResults or #stockResults == 0 then
                ...
                TriggerClientEvent('ox_lib:notify', workerId, {
                    ...
                    description = 'Not enough stock for ' ..
                        order.ingredient .. '. Allez vendre les produits demandés au marchand',
                    type = 'error',
                    position = 'top-right',
                    showDuration = true,
                    duration = 10000
            ...
            MySQL.Async.execute(
                'UPDATE warehouse_stock SET quantity = quantity - @quantity WHERE ingredient = @ingredient', {
                    ['@quantity'] = order.quantity,
                    ['@ingredient'] = order.ingredient:lower(),
                }, function(rowsChanged)
    ...
    if itemCount and itemCount >= amount then
        ...
        MySQL.Async.fetchAll('SELECT * FROM warehouse_stock WHERE ingredient = @ingredient', {
            ...
            if #stockResults > 0 then
                ...
                MySQL.Async.execute(
                    'INSERT INTO warehouse_stock (ingredient, quantity) VALUES (@ingredient, @quantity)', {
                        ['@ingredient'] = fruit,
                        ['@quantity'] = amount
                    })
            end
        ...
        TriggerClientEvent('ox_lib:notify', src, {
            title = 'Vente réussie',
            description = 'Vous avez vendu ' .. amount .. ' ' .. fruit .. ' pour $' .. total,
            type = 'success',
            duration = 9000,
            position = 'top-right'
        })
        ...
        TriggerClientEvent('ox_lib:notify', src, {
            title = 'Erreur',
            description = 'Vous n’avez pas assez de ' .. fruit,
            type = 'error',
            duration = 3000,
            position = 'top-right'
        })
