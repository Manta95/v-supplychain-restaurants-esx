            ...
            options = {
                {
                    ...
                    icon = 'fas fa-laptop',
                    label = 'Commander les Ingrédients',
                    onSelect = function()
...
AddEventHandler('restaurant:openOrderMenu', function(data)
    ...
    local isBoss = PlayerData.job and PlayerData.job.grade_name == 'boss'
    ...
    if not isBoss then
        lib.notify({
            title = 'Erreur',
            description = 'Vous n\'avez pas la permission, demandez au Boss.',
            type = 'error',
            showDuration = true,
            duration = 10000
        })
...
Citizen.CreateThread(function()
    for index, warehouse in ipairs(Config.WarehousesLocation) do
        ...
        local ped = CreatePed(4, pedModel, warehouse.position.x, warehouse.position.y, warehouse.position.z,
            warehouse.heading, false, true)
...
AddEventHandler('warehouse:openProcessingMenu', function(restaurantId)
    local playerJob = ESX.GetPlayerData().job.name
    local isAllowed = false
    local restaurantAllowPlayer = Config.Restaurants[restaurantId] and Config.Restaurants[restaurantId].allowallplayer
    --if restaurantAllowPlayer then
    for _, restaurant in pairs(Config.Restaurants) do
        if restaurant.job == playerJob or restaurant.allowallplayer == true then
            isAllowed = true
            break
        end
    end
    ...
    if not isAllowed then
        lib.notify({
            title = "Accès refusé",
            description = "Vous n'avez pas accès à ce menu d'entrepôt.",
            type = "error"
        })
        return
    end
...
AddEventHandler('warehouse:spawnVehicles', function(restaurantId, orders)
    ...
    lib.alertDialog({
        ...
        content =
        'Today you will be delivering goods to a nearby restaurant. First, back the truck into the needed zone!',
        centered = true,
        cancel = true
    })
    ...
    local truck = CreateVehicle(warehouseConfig.truck.model, warehouseConfig.truck.position.x,
        warehouseConfig.truck.position.y, warehouseConfig.truck.position.z, warehouseConfig.truck.position.w, true, false)
    local trailer = CreateVehicle(warehouseConfig.trailer.model, warehouseConfig.trailer.position.x,
        warehouseConfig.trailer.position.y, warehouseConfig.trailer.position.z, warehouseConfig.trailer.position.w, true,
        false)
    ...
    blip = AddBlipForCoord(warehouseConfig.deliveryMarker.position.x, warehouseConfig.deliveryMarker.position.y,
    ...
    Citizen.CreateThread(function()
        while true do
            ...
            DrawMarker(1, warehouseConfig.deliveryMarker.position.x, warehouseConfig.deliveryMarker.position.y,
                warehouseConfig.deliveryMarker.position.z, 0, 0, 0, 0, 0, 0, warehouseConfig.deliveryMarker.radius,
                warehouseConfig.deliveryMarker.radius, 1.0, 0, 255, 0, 100, false, true, 2, false, nil, nil, false)
            ...
            if distToMarker < warehouseConfig.deliveryMarker.radius and GetEntitySpeed(trailer) < 0.1 then
                if not notificationShown then
                    ...
                    TriggerEvent('warehouse:loadingWithForklift', warehouseConfig.trailer, warehouseConfig
                        .deliveryMarker, truck, restaurantId, orders, trailer)
                    break
                end
            end
...
AddEventHandler('warehouse:loadingWithForklift',
    function(trailerConfig, deliveryMarkerConfig, truck, restaurantId, orders, trailer)
        --print('Trailer:'.. trailer)
        lib.alertDialog({
            header = 'Nice Job!',
            content =
            'Now that the truck is ready to be loaded \n Start picking up the pallets around you and bring them towards the trailer \n Good Luck!',
            centered = true,
            cancel = true
        })
        DoScreenFadeOut(2500)
        ...
        forklift = CreateVehicle('forklift', warehouseConfig.forkliftPosition.x, warehouseConfig.forkliftPosition.y,
            warehouseConfig.forkliftPosition.z, warehouseConfig.heading, true, false)
        ...
        local forkliftReturnBlip = AddBlipForCoord(warehouseConfig.forkliftPosition.x, warehouseConfig.forkliftPosition
            .y, warehouseConfig.forkliftPosition.z)
        ...
        local truckBlip = AddBlipForCoord(deliveryMarkerConfig.position.x, deliveryMarkerConfig.position.y,
            deliveryMarkerConfig.position.z)
        ...
        Citizen.CreateThread(function()
            while true do
                ...
                DrawMarker(1, warehouseConfig.forkliftPosition.x, warehouseConfig.forkliftPosition.y,
                    warehouseConfig.forkliftPosition.z - 1.0, 0, 0, 0, 0, 0, 0, 2.0, 2.0, 1.0, 255, 0, 0, 100, false,
                    true, 2, false, nil, nil, false)
                DrawMarker(1, deliveryMarkerConfig.position.x, deliveryMarkerConfig.position.y,
                    deliveryMarkerConfig.position.z, 0, 0, 0, 0, 0, 0, deliveryMarkerConfig.radius,
                    deliveryMarkerConfig.radius, 1.0, 0, 255, 0, 100, false, true, 2, false, nil, nil, false)
                ...
                if #pallets > 0 then
                    for _, pallet in ipairs(pallets) do
                        ...
                        local deliveryZoneDist = vectorLength(vectorSubtract(GetEntityCoords(forklift),
                            deliveryMarkerConfig.position))
                                ...
                                if lib.progressCircle({
                                    ...
                                    if palletIndex > #pallets then
                                        ...
                                        lib.notify({
                                            title = 'Pallet Loaded',
                                            description =
                                            'A pallet has been loaded. Continue loading the remaining pallets.',
                                            type = 'success',
                                            showDuration = true,
                                            duration = 10000
                                        })
...
AddEventHandler('warehouse:deliverBoxes', function(restaurantId, truck, orders, trailer)
    ...
    Citizen.CreateThread(function()
        ...
        palletProp = CreateObject(palletModel, trailerBackPosition.x, trailerBackPosition.y, trailerBackPosition.z, true,
            true, true)
...
AddEventHandler('warehouse:returnTruck', function(truck, restaurantId, orders)
    ...
    --print("Received orders:", json.encode(orders))
    local playerPed = PlayerPedId()
    local truckReturnPosition = vector3(Config.Warehouses[1].truck.position.x, Config.Warehouses[1].truck.position.y,
        Config.Warehouses[1].truck.position.z)
